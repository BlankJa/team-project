...
public class GeoApifyGatewayImpl implements PlacesDataAccessInterface {
    ...
    public GeoApifyGatewayImpl(PlacesApiLogger logger) {
        this.apiKey = "75be457789934a199ed4014ad24925ba";
        this.logger = logger;
    }

    public GeoApifyGatewayImpl(String apiKey, PlacesApiLogger logger) {
        this.apiKey = apiKey;
        this.logger = logger;
    }
    ...
    public List<Place> searchPlaces(double lat, double lon, double radiusKm,
        ...
        String location = String.format("(%.4f, %.4f) radius: %.1fkm", lat, lon, radiusKm);
        if (logger != null) {
            logger.logSearchRequest(location, "N/A");
        }
        ...
        try {
            ...
            if (logger != null) {
                logger.logError(e.getMessage() != null ? e.getMessage() : e.getClass().getName());
            }
            throw e;
        }
    }
